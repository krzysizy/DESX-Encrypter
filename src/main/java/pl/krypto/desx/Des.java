...
public class Des {
    ...
    {
        byte[] activeKey = Permutation.permutation(key, PC1);
        int halfKeySize = PC1.length / 2;  //28 bitów
        ...
        for (int k = 0; k < 16; k++)
        {
            c = BitOperations.shiftLeft(c, SHIFTS[k]);
            d = BitOperations.shiftLeft(d, SHIFTS[k]);
            byte[] cd = BitOperations.joinTabBytes(c, d);
            subKeysLocal[k] = Permutation.permutation(cd, PC2);
        }
        return subKeysLocal;
    }
